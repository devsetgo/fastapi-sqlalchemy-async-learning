...
1. Move to the directory where `pyproject.toml` is located:
   ...
   ```shell
   cd fastapi-sqlalchemy-async-learning
   ```
...
6. Open your browser and go to `http://localhost:5000` to see the app running.
...
7. Since there is only one SQLAlchemy model, you can create a new migration file by running the following command:
   ...
   This will create a new migration file in `fastapi-sqlalchemy-async-learning/alembic/versions/`.
...
8. To check the documentation of the API, go to `http://localhost:5000/docs`.
